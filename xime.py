    ...
    for file_name, text in texts.items():
        with st.spinner(f'Generating summary for {file_name}...'):
            prompt = f'''Role: You are an AI assistant trained in legal expertise and your answers needs to be always in Spanish and just provide the text requested no need of titles
                        ...
                        Task 1: 
                        Create a summary of approximately 100 words for the {text} and present it after writing "Resumen:" 
                        Task 2: 
                        Identify and extract all the legal arguments in the document and write them after writing "Argumentos legales"'''

            summary = create_summary(prompt)

            # Split the summary at 'Resumen:'
            _, _, remaining_text = summary.partition('Resumen:')

            # Split the remaining text at 'Argumentos legales:'
            summary_100_words, _, legal_arguments = remaining_text.partition('Argumentos legales:')

            # Add the information to the DataFrame
            new_row = pd.DataFrame({'Name of the document': [file_name], 'Summary in 100 words': [summary_100_words.strip()], 'Main legal arguments': [legal_arguments.strip()]})
